...
class AccessControlMiddleware(BaseMiddleware):
    """Ограничивает доступ к боту только для разрешенных Telegram ID."""
    ...
    async def __call__(
        ...
        if user_id is not None and user_id not in self.allowed_user_ids:
            logger.warning(
                "Доступ заблокирован для telegram_id=%s. Список разрешенных: %s",
                user_id,
                self.allowed_user_ids,
            )
            return None  # Молча игнорируем (Stealth Mode)
